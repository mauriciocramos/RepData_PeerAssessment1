...
The installed packages versions used in this research are dplyr `r packageVersion("dplyr")`,tidyr `r packageVersion("tidyr")`, ggplot2 `r packageVersion("ggplot2")`, lubridate `r packageVersion("lubridate")` and impute `r packageVersion("impute")`.
...
ggplot(daily, aes(x = total.steps)) +
    ...
    ylab("percentage") + xlab("total steps each day") +
...
```{r Average daily activity pattern in 5 minutes intervals}
...
intervalMatrix <- activity %>%
    ...
    as.matrix
```
This new layout features `r nrow(intervalMatrix)` days as observations and `r ncol(intervalMatrix)` five-minutes intervals as variables.
...
```{r Heatmap of daily activity pattern in 5 minutes intervals}
...
```{r Total number of missing data intervals each day}
...
```{r Histogram of the missing data days per week day}
breaks <- wday(breaks)
...
```{r Histogram of the total number of steps taken each with mean imputation}
...
```{r Histogram of the total number of steps taken each day with K-NN imputation}
