...
```
This research requires additional R packages.The installed packages versions used in this research are dplyr `r packageVersion("dplyr")`,tidyr `r packageVersion("tidyr")`, ggplot2 `r packageVersion("ggplot2")`, lubridate `r packageVersion("lubridate")` and impute `r packageVersion("impute")`.
...
```{r Frequency-histogram-steps-each-day}
...
```{r Density-histogram-steps-each-day}
ggplot(daily, aes(x = total.steps)) +
...
Transform the long-form (`r nrow(activity)` by `r length(activity)`) dataset in a wider matrix.
...
This new data format features `r nrow(intervalMatrix)` days as observations and `r ncol(intervalMatrix)` five-minutes intervals as variables.
...
2. They happen on the first (`r labels[1]`) and last (`r labels[8]`) days of the experiment.
3. They happen on the first two Mondays: `r labels[1]` and `r labels[2]`.
4. They happen on two adjacent days only once: `r labels[5]` and `r labels[6]`.
...
Mean of the total number of steps taken per day      |`mean(daily$total.steps, na.rm = TRUE)/17280`|`r mean(daily$total.steps, na.rm = TRUE)/17280`|
Median of the total number of steps taken per day    |`median(daily$total.steps, na.rm = TRUE)/17280`|`r median(daily$total.steps, na.rm = TRUE)/17280`|
...
TODO: REVISE
As there is missing data only on `r missingDays` full days, the maximum *k*-neighbors possible are `r (nrow(missingDaily) - missingDays)`.
...
TODO: REVISE
In order to avoid the bias of using overall mean per sample, `rowmax` should never be reached.  As there are full rows (days) with missing values, `rowmax` should be `r maxMissingIntervals`/`r nrow(fiveminutes)`, `r (maxMissingIntervals/nrow(fiveminutes))`.  A general formula to optimize `rowmax` is dividing the maximum number of missing values in a row by the number of columns:
...
TODO: REVISE
As there is missing data only in `r missingDays` full days (rows) of `r (nrow(daily))` days,  the `colmax` should be at minimum the `r missingDays`/`r nrow(daily)` ratio, `r (missingDays/nrow(daily))`.  A general formula to ensure a minimum `colmax` is dividing the maximum number of missing values in a column by the number of rows:
...
```{r Histogram-steps-each-day-mean-imputation}
...
```{r HHistogram-steps-each-day-mean-KNN-imputation}
